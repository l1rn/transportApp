...
- [💾Redis](#redis)
- [👤Несколько профилей запуска](#несколько-профилей-для-запуска-)
- [📮Коллекции в Postman](#теперь-можно-загрузить-коллекции-в-postman)
- [🟢Управление в профиле](#теперь-можно-управлять-сессиями)
...
# Несколько профилей для запуска 👤
1. Как обычно нужно будет открыть директорию `/transportApp/backend`
2. Если хотите выбрать профиль 
```cmd
# По дефолту у нас будет стоять профиль на разработку
mvn spring-boot:run
# Ну или точечный старт, если будет много профилей, чтобы не запутаться
mvn spring-boot:run -P dev

# Удалить все файлы и пересоберет без тестов
mvn clean package -P test-profile -DskipTests

# Запустит на тестовом профиле
mvn spring-boot:run -P test-profile
```
## Что нужно учитывать при запуске профилей ✍️
Для профилей, будут разные конфигурации. Вот все возможные на данный момент достуные профили для.
### Параметры токенов:
| profile     | ttl-accessToken|
|-------------|----------|
| dev         | `1min`   | 
| test-profile| `10sec`  |
| prod        | `10min`  | 
### Параметры для работы с консолью:
| profile     | JDBC-URL                             | username |  password |
|-------------|--------------------------------------|----------|-----------|
| dev         | `jdbc:h2:mem:testtransportdb`        | `sa`     |           |
| test-profile| `jdbc:h2:file:./data/transport_db`   | `root`   |  `root`   |
| prod        | `jdbc:h2:file:/app/data/transport_db`| `root`   |  `root`   |
...
# Теперь можно управлять сессиями🟢
Можно по одному клику в профиле удалить вашу прошлую сессию на любом устройстве
![](documentation/images/11.png)
## Смена пароля
Вы можете сменить пароль, при вводе старого и поменять на новый
